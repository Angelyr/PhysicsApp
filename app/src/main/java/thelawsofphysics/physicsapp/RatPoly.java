...
public final class RatPoly {
    ...
    private static void scaleCoeff(RatNum[] arr, RatNum scalar) {
        if (scalar==RatNum.ZERO) arr=null;
    ...
    public RatPoly negate() {
        ...
        if (this.coeffs.length==1 && coeffs[0].intValue()==0){
            return new RatPoly();
        }
        if (this.coeffs.length==0){
            return ZERO;
        }
        RatNum[] k=coeffs.clone();
        scaleCoeff(k, new RatNum(-1));
        return new RatPoly(k);
    ...
    public RatPoly add(RatPoly p) {
        ...
        RatNum[]ncoeffs= new RatNum[Math.max(this.degree()+1,p.degree()+1)];
        for (int i = 0; i < ncoeffs.length; i++)
            ncoeffs[i] = new RatNum(0);
        for (int i=0;i<=this.degree();i++){
            ncoeffs[i]= ncoeffs[i].add(this.getCoeff(i));
        }
        for (int i=0;i<=p.degree();i++){
            ncoeffs[i]=ncoeffs[i].add(p.getCoeff(i));
        }

        int Largestterm=0;
        for (int i=ncoeffs.length-1;i>=0;i--){
            if (ncoeffs[i].intValue()!=0){
                Largestterm=i;
                break;
            }
        }

        RatNum[] n = new RatNum[Largestterm+1];
        for (int i=0;i<n.length;i++){
            n[i]=ncoeffs[i];
        }
        if (n.length==1 && n[0].intValue()==0) {
            return new RatPoly();
        }
        if (n.length==0){
            return new RatPoly();
        }
        return new RatPoly(n);
    ...
    public RatPoly mul(RatPoly p) {
        ...
        if ((this.getCoeff(0).intValue()==0 && this.degree()==0) || (p.getCoeff(0).intValue()==0 && p.degree()==0)){
            return new RatPoly();
        }
        RatNum[]ncoeffs= new RatNum[this.degree()+p.degree()+1];
        for (int i = 0; i < ncoeffs.length; i++)
            ncoeffs[i] = new RatNum(0);
        for (int i=0; i <=this.degree();i++){
            for (int j=0; j<=p.degree();j++){
                ncoeffs[i+j]= ncoeffs[i+j].add(this.coeffs[i].mul(p.getCoeff(j)) );
            }
        }
        if (ncoeffs.length==1 && ncoeffs[0].intValue()==0){
            return new RatPoly();
        }
        return new RatPoly(ncoeffs);
    }
    /**
     * Division operation.
     *
    ...
    public RatPoly div(RatPoly p){
        ...
        if (p.getCoeff(0).intValue()==0 && p.degree()==0){
            RatNum[] r = new RatNum[1];
            r[0] =  new RatNum(1,0);
            return new RatPoly(r);
        }
        ...
        RatNum c= this.getCoeff(this.degree()).div(p.getCoeff(p.degree()));
        int exp= this.degree()-p.degree();
        RatPoly r=new RatPoly(c.intValue(),exp);
        return r.add((this.sub( p.mul(r) ).div(p)) );
    ...
    public double eval(double d) {
        ...
        for (int i=this.degree(); i>=0;i--){
            r= this.coeffs[i].doubleValue() + r*d;
        }
        return r;
    ...
    public RatPoly differentiate() {
        ...
        if (this.degree()==0){
            ...
            RatNum[]ncoeffs= new RatNum[this.degree()];
            for (int i = 0; i < ncoeffs.length; i++)
                ncoeffs[i] = new RatNum(0);
            for (int i = 0; i < this.degree(); i++){
                RatNum c= new RatNum(i+1);
                ncoeffs[i] =  this.coeffs[i + 1].mul(c);
            }
            return new RatPoly(ncoeffs);
        }
    }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        if (this.isNaN()==true || integrationConstant.isNaN()==true){
            RatNum[] r = new RatNum[1];
            r[0] =  new RatNum(1,0);
            return new RatPoly(r);
        }
        if (this.coeffs.length==0){
            RatNum[] ncoeffs= new RatNum[1];
            if (integrationConstant.intValue()==0){
                return new RatPoly();
            }
            else{
                ncoeffs[0]=integrationConstant;
                return new RatPoly(ncoeffs);
            }
        }
        RatNum[]ncoeffs= new RatNum[this.degree()+2];
        for (int i = 0; i < ncoeffs.length; i++)
            ncoeffs[i] = new RatNum(0);
        ncoeffs[0]= integrationConstant;
        for (int i = 0; i <= this.degree(); i++){
            if (coeffs[i].intValue()!=0){
                RatNum c= new RatNum(i+1);
                ncoeffs[i+1] =  this.coeffs[i].div(c);
            }
        }
        return new RatPoly(ncoeffs);
    }
    ...
    public double integrate(double lowerBound, double upperBound) {
        ...
        return a.eval(upperBound)-(a.eval(lowerBound));
    }
    ...
    public static RatPoly valueOf(String polyStr) {
        ...
        for( int i=0; i<polyStr.length(); i++ ) {
            if( polyStr.charAt(i) == 'x' && i!=0 ) {
                boolean a =Character.isDigit(polyStr.charAt(i-1));
                if  (a == true){ return RatPoly.NaN;}
            }
        }
